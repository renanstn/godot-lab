...
void fragment()
{
	...
	COLOR = texture(SCREEN_TEXTURE, vec2(xpos, uv.y));
    ...
    COLOR.g = mix(COLOR.r, texture(SCREEN_TEXTURE, vec2(xpos + noise * 0.05, uv.y)).g, 0.25);
    COLOR.b = mix(COLOR.r, texture(SCREEN_TEXTURE, vec2(xpos - noise * 0.05, uv.y)).b, 0.25);
}